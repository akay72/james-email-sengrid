...
# Function to insert data into SQL Server, ensuring msg_id is unique, with feedback on skipped insertions
def insert_into_sql(data):
    conn_str = f'DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server};DATABASE={database};UID={username};PWD={password}'
    with pyodbc.connect(conn_str) as conn:
        cursor = conn.cursor()
        for message in data:
            cursor.execute(f"SELECT 1 FROM {table} WHERE msg_id = ?", message.get("msg_id"))
            if cursor.fetchone():
...
# Function to fetch emails with status 'delivered'
def fetch_emails_with_status_delivered():
    conn_str = f'DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server};DATABASE={database};UID={username};PWD={password}'
    emails_to_resend = []
    with pyodbc.connect(conn_str) as conn:
        cursor = conn.cursor()
        select_query = f"SELECT msg_id, from_email, to_email FROM {table} WHERE status <> 'delivered' AND send_email_again <> 1"
        cursor.execute(select_query)
        columns = [column[0] for column in cursor.description]
        for row in cursor.fetchall():
            emails_to_resend.append(dict(zip(columns, row)))
...
def send_email_with_reason(sendgrid_api_key, from_email, to_email):
    ...
    # return new_message_id  # Return the new message ID

def update_database_with_new_message_id(conn_str, original_msg_id, new_msg_id):
    with pyodbc.connect(conn_str) as conn:
        cursor = conn.cursor()
        update_sql = f"""
            UPDATE {table}
            SET new_message_id = ?, send_email_again = 1
            WHERE msg_id = ?
        """
...
try:
    ...
    if 200 <= response.status_code < 300:
        ...
        for email_info in delivered_emails:
            ...
            # Update the database with the new message ID and set send_email_again to True
